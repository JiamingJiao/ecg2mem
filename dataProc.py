...
def generatePseudoECG(srcPath, dstPath, electrodesNum, zDistance = 100, interpolationSize = (200, 200)):
    ...
    dst = np.ndarray((src.shape[1], src.shape[2]), dtype = np.float64)
    ...
    for i in range(0, src.shape[0]):
        ...
        for siteNum in range(0, electrodesNum):
            cellRow = math.floor(siteNum/math.sqrt(electrodesNum)+1e-5)
            cellCol = siteNum%math.floor(math.sqrt(electrodesNum)+1e-5)
